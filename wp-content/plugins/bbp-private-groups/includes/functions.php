...
function pg_get_forum_freshness_link_anchor( $forum_id = 0 ) {
	...
	if ( !empty( $time_since ) && !empty( $link_url ) ) {
			...
			if (private_groups_can_user_view_post($user_id,$forum_id) &&  !bbp_is_forum_private($forum_id)) {
			...
			elseif (private_groups_can_user_view_post($user_id,$forum_id) && bbp_is_forum_private($forum_id) && current_user_can( 'read_private_forums' ) ) {
				$anchor ['link_url'] = $link_url ;
				$anchor ['title'] = $title;
				$anchor ['display'] = $time_since;
			}
